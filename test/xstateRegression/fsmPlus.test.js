...
describe('@xstate/fsm', () => {
    ...
    it('should have the correct initial state', () => {
        ...
        expect(initialState.value).toEqual('#light.green');
    ...
    it('should transition correctly', () => {
        ...
        expect(nextState.value).toEqual('#light.yellow');
    ...
    it('should stay on the same state for undefined transitions', () => {
        ...
        expect(nextState.value).toBe('#light.green');
    ...
    it('should work with guards', () => {
        ...
        expect(yellowState.value).toEqual('#light.yellow');
        const redState = lightFSM.transition('yellow', {
            type: 'EMERGENCY',
            value: 2
        });
        expect(redState.value).toEqual('#light.red');
        ...
        expect(redOneState.value).toBe('#light.red');
    ...
    it('should match transition states', () => {
        ...
        expect(nextState.matches('#light.yellow')).toBeTruthy();
...
describe('interpreter', () => {
    ...
    it('listeners should subscribe to state changes', (done) => {
        const toggleService = interpret(toggleMachine).start();
        toggleService.subscribe((state) => {
            if (state.matches('#toggleMachine.inactive')) {
                done();
            }
        });
    ...
    describe('`start` method', () => {
        it('should start the service with initial state by default', () => {
            ...
            expect(service.state.value).toBe('#testMachine03.foo');
        });
        it('should rehydrate the state if the state if provided', () => {
            ...
            expect(service.state.value).toBe('#testMachine04.bar');
            service.send('NEXT');
            expect(service.state.matches('#testMachine04.baz')).toBe(true);
        });
        it('should rehydrate the state and the context if both are provided', () => {
            ...
            expect(service.state.value).toBe('#testMachine05.bar');
            expect(service.state.context).toBe(context);
            service.send('NEXT');
            expect(service.state.matches('#testMachine05.baz')).toBe(true);
        });
    });
    ...
    it('should reveal the current state', () => {
        ...
        service.start();
        expect(service.state.value).toEqual('#testMachine08.test');
    ...
    it('should reveal the current state after transition', (done) => {
        ...
        service.start();
        service.subscribe(() => {
            if (service.state.value === '#testMachine09.success') {
                done();
            }
        });
        service.send('CHANGE');
